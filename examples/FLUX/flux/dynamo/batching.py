...
def batch(  # noqa: C901
    ...
    .. code-block:: python
            ...
            class BatchedDeployment:
                ...
                @flux.dynamo.batch(max_batch_size=2, batch_wait_timeout_s=0.1)
    ...
    def _batch_decorator(_func):
        ...
        def enqueue_request(args, kwargs) -> asyncio.Future:
            ...
            future = _get_or_create_event_loop().create_future()
            batch_queue.put(_SingleRequest(self, flattened_args, future))  # type: ignore
            return future
        ...
        # FIXME: remove, no need for runtime changes
...
class _BatchQueue:
    ...
    def _warn_if_max_batch_size_exceeds_max_ongoing_requests(self):
        ...
        """
        max_ongoing_requests = MAX_ONGOING_REQUESTS  # TODO: get workerer configuration
        if max_ongoing_requests < self.max_batch_size:
            logger.warning(
                "`max_batch_size` (%d) is larger than `max_ongoing_requests` (%d). This means the replica will never receive a full batch. Please update `max_ongoing_requests` to be >= `max_batch_size`.",
                self.max_batch_size,
                max_ongoing_requests,
            )

    def set_max_batch_size(self, new_max_batch_size: int) -> None:  # FIXME: remove, no need for runtime changes
        """Updates queue's max_batch_size."""
        self.max_batch_size = new_max_batch_size
        self._warn_if_max_batch_size_exceeds_max_ongoing_requests()

    def put(self, request: tuple[_SingleRequest, asyncio.Future]) -> None:  # FIXME tuple ?!
        self.queue.put_nowait(request)  # type: ignore
            ...
            async for results in func_generator:
                ...
                for idx in range(input_batch_length):
                    ...
                    if future is FINISHED_TOKEN:
                        ...
                        next_future = _get_or_create_event_loop().create_future()
    ...
    def __del__(self):
        if self._handle_batch_task is None or not _get_or_create_event_loop().is_running():
            return
...
class _LazyBatchQueueWrapper:
    ...
    def _get_curr_iteration_start_time(self) -> float | None:
    ...
    async def _get_handling_task_stack(self) -> str | None:

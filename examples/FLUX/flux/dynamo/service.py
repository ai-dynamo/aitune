...
"""AI Dynamo service with FLUX model and Ray Serve batching.

This service implements batched image generation using Ray Serve's @serve.batch decorator
...
from .batching import _get_or_create_event_loop, batch
...
class FluxBatchedBackend:
    """Backend service for FLUX model inference with Ray Serve batching."""

    def __init__(self) -> None:
        """Initialize the FLUX backend with Ray Serve batching."""
        logger.info("Starting FLUX backend with Ray Serve batching")
    ...
    async def handle_batch(self, requests: list[ImageGenerationRequest]) -> list[ImageGenerationResponse]:
        """Process a batch of image generation requests using Ray Serve batching."""
        if not requests:
            return []
        ...
        try:
            ...
            for i, req in enumerate(requests):
                ...
                try:
                    ...
                    images[j].save(img_buffer, format="JPEG")  # type: ignore
            ...
            logger.error("Error processing batch: %s", e, exc_info=e)
    ...
    async def generate_image(self, req: ImageGenerationRequest):
        """Generate image with Ray Serve batching."""
        ...
        # Process through batch handler - Ray Serve will automatically batch this
        try:
...
class FluxBatchedFrontend:
    """Frontend HTTP API for FLUX inference service with Ray Serve batching."""
    ...
    def __init__(self) -> None:
        ...
        logger.info("Starting FLUX frontend with Ray Serve batching")
    ...
    async def generate_image(self, request: ImageGenerationRequest):
        """HTTP endpoint for image generation with Ray Serve batching."""
    ...
    async def health(self):
        ...
        return {"status": "healthy", "service": "FLUX Image Generation with Ray Serve Batching"}

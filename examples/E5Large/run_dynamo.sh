...
python -m e5large.dynamo.backend & # (2) start the backend in the background
...
for i in {1..5}; do
  ...
  echo "Waiting for intfloat/e5-large-v2 to appear in /v1/models... (attempt $i)"
  sleep 1
done

python -m e5large.dynamo.client --sentence "Hello, world!" # (3) run the test script

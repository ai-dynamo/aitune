...
"""Client example for Stable Diffusion batching service."""
...
class StableDiffusionClient:
    """Client for Stable Diffusion batching service."""

    def __init__(self, base_url: str = "http://localhost:8000"):
        """Initialize the client.

        Args:
            base_url: Base URL of the Stable Diffusion service
        """
...
async def demo_batching(num_requests: int = 4):
    ...
    async with StableDiffusionClient() as client:
        # Check health
        # health = await client.health_check()
        health = {"status": "healthy", "service": "Stable Diffusion Image Generation"}
        ...
        for i, result in enumerate(successful_results):
            try:
                ...
                filename = f"sd_batch_{i:02d}_{result['request_id'][:8]}.jpg"
...
async def demo_single_request():
    ...
    async with StableDiffusionClient() as client:
        ...
        try:
            ...
            filename = f"sd_single_{result['request_id'][:8]}.jpg"
            image_path = output_dir / filename
            image.save(image_path)
            logger.info("Saved image: %s", image_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Stable Diffusion client demo, sends given number of requests/prompts to the service",
        epilog="PROMPTS: \n{}\n".format(", ".join(PROMPTS)),
    )
    parser.add_argument(
        "-n",

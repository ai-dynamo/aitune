...
"""StableDiffusion image backend — serves via dyn.dynamo_worker."""
...
def main() -> None:
    ...
    def generate(prompt: str, height: int, width: int, num_inference_steps: int, guidance_scale: float) -> bytes:
        result = pipeline(
            prompt=prompt,
            height=height,
            width=width,

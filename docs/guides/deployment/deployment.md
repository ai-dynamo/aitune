...
Triton repository from an ONNX Runtime, TensorRT, or TorchInductor AOT artifact, or publish an existing ONNX, TensorRT,
or PT2 file with an explicit model configuration. Requests use the KServe gRPC tensor API.
...
[standalone Triton](triton.md#run-triton-standalone). You can generate a repository for an optimized artifact or an
existing model file, or provide a Python backend that loads an AITune checkpoint. All paths are covered in the
[Triton guide](triton.md).
...
| Triton loading an optimized artifact or existing model file directly | A generated Triton model repository | Dynamo's Triton worker or standalone Triton |

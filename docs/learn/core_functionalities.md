...
Strategies automatically validate performance against a Torch eager baseline so an optimization that does not improve
performance is not selected. Most users do not need to configure this behavior. See
[Performance Validation](../guides/advanced/tuning_workflow.md#performance-validation) for diagnostic analytics and
advanced controls.

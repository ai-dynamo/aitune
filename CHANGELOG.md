...
- feat: add the experimental `KernelSelectorBackend` for selecting functional kernel providers before building a delegate JIT or AOT backend
- feat: add the experimental `KernelSelector` API with Torch SDPA, SageAttention, and FlashAttention-4 providers and serializable provider runtime plans

...
from aitune.torch.utils.pt2_artifact import PT2CallContract, PT2CallContractState, create_pt2_artifact
...
class TorchInductorAotBackend(Backend):
    ...
    def __init__(self, config: TorchInductorAotBackendConfig | None = None):
        ...
        self._artifact: PT2Artifact | None = None
    ...
    def artifact(self) -> PT2Artifact:
        """Return the deployable AOTInductor package and its ordinal tensor contract."""
        if self._artifact is None:
            detail = f": {self._artifact_error}" if self._artifact_error else ""
            raise RuntimeError(f"PT2 artifact is not available until the backend has built or deployed{detail}")
    ...
    def _activate(self):
        ...
        if self._artifact is None and self._pt2_call_contract is not None:
            self._capture_artifact()

    def _capture_artifact(self) -> None:
        """Capture publication metadata without changing backend activation success."""
        try:
            if self._compiled_model_artifact is None or self._graph_spec is None or self._pt2_call_contract is None:
                raise RuntimeError("PT2 artifact requires a compiled package, graph specification, and call contract")
            self._artifact = create_pt2_artifact(
                self._graph_spec,
                self._compiled_model_artifact.path,
                self._pt2_call_contract,
            )
            self._artifact_error = None
        except (OSError, RuntimeError, ValueError) as error:
            self._artifact_error = str(error)
            logger.info("PT2 package is not available for deployment: %s", error)

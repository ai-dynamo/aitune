...
@dataclass
class KernelOptimizerBackendConfig(BackendConfig):
    """Configuration for kernel optimization performed before delegate build.
    ...
    def describe(self) -> str:
        """Return a human-readable optimizer configuration."""
    ...
    @classmethod
    def from_dict(cls, _data: dict) -> "KernelOptimizerBackendConfig":
...
class KernelOptimizerBackend(Backend):
    """Apply a selected kernel optimization plan while building a delegate backend.
    ...
    def key(self) -> str:
        """Return a cache key combining optimizer and delegate configuration."""
    ...
    def describe(self) -> str:
        """Describe optimizer and delegate configuration."""
        if self._config is None:
    ...
    @classmethod
    def from_dict(cls, module: nn.Module | None, state_dict: dict) -> "KernelOptimizerBackend":
        ...
        if state_dict.get(cls.STATE_VERSION) != cls.state_version:
            raise ValueError(f"Unsupported KernelOptimizerBackend state version: {state_dict.get(cls.STATE_VERSION)}")

        delegate_state = state_dict.get(cls.STATE_DELEGATE_BACKEND)
        if not isinstance(delegate_state, dict):
            raise ValueError("KernelOptimizerBackend state requires a delegate backend")
    ...
    def _delegate_class_from_state(state_dict: dict[str, Any]) -> type[Backend]:
        ...
        backend_classes = {
            ...
            for backend_class in KernelOptimizerBackend._backend_subclasses(Backend)
        }
        try:
    ...
    def _backend_subclasses(backend_class: type[Backend]) -> list[type[Backend]]:
        ...
        for subclass in backend_class.__subclasses__():
            ...
            subclasses.extend(KernelOptimizerBackend._backend_subclasses(subclass))
        return subclasses

...
def prepare_calibration_data(data: list[Sample], graph_spec: GraphSpec) -> dict[str, np.ndarray]:
    ...
    for ONNX input names and locators into (args, kwargs).
    ...
    onnx_input_names = [tensor_spec.name for _, tensor_spec in tensor_data]
    logger.info(
    ...
    per_sample = _collect_per_sample_dicts(data, tensor_data)
    if not per_sample:
        raise ValueError(
...
def _sample_to_input_dict(
    ...
    for locator, tensor_spec in tensor_data:
        value = locator.get_value(args) if tensor_spec.name.startswith("args") else locator.get_value(kwargs)
        if not torch.is_tensor(value):
            ...
            continue
        input_dict[tensor_spec.name] = value.detach().cpu().numpy()
    return input_dict

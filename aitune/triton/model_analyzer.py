...
_QUICK_CONFIG_FILE_NAME = "config.yaml"
...
class _DeploymentQuickConfig(_ConfigModel):
    """Main workflow's bounded quick search with an optional latency budget."""
...
def write_model_analyzer_config(
    ...
    *,
    ...
    """Write the quick-search configuration used by the Triton deployment example."""
    shapes = {tensor.name: tensor.min_shape for tensor in artifact.inputs}
    flags = tuple(
        f"{name}:{','.join(str(dimension) for dimension in (shape[1:] if config.max_batch_size else shape))}"
        for name, shape in shapes.items()
    )
    ...
    (output_directory / _QUICK_CONFIG_FILE_NAME).write_text(analyzer_config.to_yaml())
    if input_data is not None:

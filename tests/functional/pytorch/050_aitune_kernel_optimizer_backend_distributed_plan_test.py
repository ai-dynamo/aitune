...
from aitune.torch.backend import KernelOptimizerBackend, KernelOptimizerBackendConfig, TorchEagerBackend
...
def _run_worker(device: torch.device) -> None:
    ...
    backend = KernelOptimizerBackend(
        KernelOptimizerBackendConfig(kernel_providers=RankPlanProvider(0.0)),
        TorchEagerBackend(),
    )

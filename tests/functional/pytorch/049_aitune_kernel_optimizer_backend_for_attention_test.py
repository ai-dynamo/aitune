...
def test_kernel_optimizer_backend_for_attention(delegate_backend: Backend):
    ...
    # ------------------ kernel optimizer --------------
    create_optimizer = KernelOptimizerBackend._create_optimizer
    selected_plans = []
    ...
    # -------------------- tune --------------------
    with patch.object(KernelOptimizerBackend, "_create_optimizer", create_preferred_optimizer):
    ...
    assert isinstance(built_backend, KernelOptimizerBackend)
    ...
    assert isinstance(restored_backend, KernelOptimizerBackend)
...
if __name__ == "__main__":
    ...
    for delegate_backend in (inductor_jit, inductor_aot, tensorrt_jit):
        test_kernel_optimizer_backend_for_attention(delegate_backend)

...
from aitune.torch.backend import ONNXAutoCastConfig, TensorRTBackend, TensorRTBackendConfig
...
def do_test(backend: TensorRTBackend, dtype: torch.dtype):
    # given
    ...
    do_test(
        TensorRTBackend(
            TensorRTBackendConfig(
                quantization_config=ONNXAutoCastConfig(),
            )
        ),
        torch.float32,
    )

...
def test_stable_diffusion_dynamic_batch_onnx_runtime():
    """Test ONNX Runtime backend (CUDA EP) with StableDiffusion pipeline.

    Mirrors test 012 (TensorRT) but replaces TensorRTBackend with
    ONNXRuntimeBackend so the same lifecycle is exercised:
        inspect → wrap → tune (batch_sizes=[1,2]) → save → load → infer

    Two batch sizes are verified (bs=1 and bs=2) to confirm that the
    dynamic_axes exported to the ONNX model correctly support variable
    batch dimensions.
    """
    ...
    logger.info("Starting StableDiffusion ONNX Runtime test")

    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        tuned_model_path = temp_path / "tuned_stable_diffusion_onnx.pt"
        ...
        try:
            ...
            actual_bs1[0].save(output_dir / "sd_onnx_bs1.jpg")
            ...
            for i, image in enumerate(actual_bs2):
                image.save(output_dir / f"sd_onnx_bs2_{i}.jpg")
            ...
            logger.info("All ONNX Runtime StableDiffusion tests passed")
...
if __name__ == "__main__":
    test_stable_diffusion_dynamic_batch_onnx_runtime()

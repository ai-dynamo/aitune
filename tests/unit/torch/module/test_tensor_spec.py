...
def test_tensor_spec_from_tensor():
    spec = TensorSpec.from_tensor(torch.randn(1, 2, 3), batch_size=1)
...
def test_tensor_spec_hash():
    spec1 = TensorSpec.from_tensor(torch.randn(1, 2, 3), batch_size=1)
    spec2 = TensorSpec.from_tensor(torch.randn(3, 2, 1), batch_size=1)
    assert hash(spec1) == hash(spec2)
    assert spec1 == spec2
    spec3 = TensorSpec.from_tensor(torch.randn(4, 2, 4), batch_size=1)
    assert hash(spec1) == hash(spec3)
    assert spec1 == spec3
    spec4 = TensorSpec.from_tensor(torch.randn(1, 2), batch_size=1)
...
def test_tensor_spec_info():
    spec = TensorSpec.from_tensor(torch.randn(1, 2, 3), batch_size=1)
    assert spec.describe(InfoLevel.SHORT) == "shape=[1, 2, 3]"
    assert spec.describe(InfoLevel.MEDIUM) == "shape=[1, 2, 3] dtype=torch.float32"
    assert (
        spec.describe(InfoLevel.FULL) == "shape=[1, 2, 3] min_shape=[1, 2, 3] max_shape=[1, 2, 3] dtype=torch.float32"
    )
    assert str(spec) == "shape=[1, 2, 3]"
    assert repr(spec) == "shape=[1, 2, 3] dtype=torch.float32"


def test_tensor_spec_update_shapes_seen():
    # case 1
    spec1 = TensorSpec.from_tensor(torch.randn(1, 2, 3), batch_size=1)
    spec2 = TensorSpec.from_tensor(torch.randn(3, 2, 6), batch_size=3)
    ...
    # case 3
    spec4 = TensorSpec.from_tensor(torch.randn(5, 4, 10), batch_size=5)
    ...
    # case 4
    spec5 = TensorSpec.from_tensor(torch.randn(1, 2), batch_size=1)
...
def test_has_batch_axis():
    spec1 = TensorSpec.from_tensor(torch.randn(1, 2), batch_size=1)
    spec2 = TensorSpec.from_tensor(torch.randn(3, 2), batch_size=3)
    spec1.update_shapes_seen(spec2)
...
def test_has_dynamic_axis():
    spec1 = TensorSpec.from_tensor(torch.randn(1, 2), batch_size=1)
    spec2 = TensorSpec.from_tensor(torch.randn(2, 2), batch_size=3)
    spec1.update_shapes_seen(spec2)
...
def test_matches():
    ...
    spec1 = TensorSpec.from_tensor(torch.randn(1, 2), batch_size=1)
    spec2 = TensorSpec.from_tensor(torch.randn(1, 2), batch_size=1)
    ...
    spec1 = TensorSpec.from_tensor(torch.randn(1, 2), batch_size=1)
    spec2 = TensorSpec.from_tensor(torch.randn(1, 3), batch_size=1)
    assert not spec1.matches(spec2)
    assert not spec2.matches(spec1)

    # Test case 5: Different ranks - should not match
    spec1 = TensorSpec.from_tensor(torch.randn(1, 2), batch_size=1)
    spec2 = TensorSpec.from_tensor(torch.randn(1, 2, 3), batch_size=1)
    assert not spec1.matches(spec2)
    assert not spec2.matches(spec1)

    # Test case 6: Scalars i.e. empty shapes
    spec1 = TensorSpec.from_tensor(torch.randn(1), batch_size=1)
    spec2 = TensorSpec.from_tensor(torch.randn(1), batch_size=1)
    spec1.shape = []
    spec2.shape = []
    assert spec1.matches(spec2)
    assert spec2.matches(spec1)

...
"""Tests for the composite kernel selector backend."""
...
def _backend(delegate, optimizer):
    ...
    backend = KernelSelectorBackend(
        config=KernelSelectorBackendConfig(kernel_providers=source),
...
def test_config_defaults_to_attention_providers():
    first_config = KernelSelectorBackendConfig()
    second_config = KernelSelectorBackendConfig()
...
def test_config_requires_and_normalizes_explicit_kernel_sources():
    with pytest.raises(ValueError, match="At least one kernel provider or generator must be provided"):
        KernelSelectorBackendConfig(kernel_providers=None)

    provider = _ReluProvider()
    provider_config = KernelSelectorBackendConfig(kernel_providers=provider)
    generator = _ReluGenerator()
    generator_config = KernelSelectorBackendConfig(kernel_providers=None, kernel_generators=generator)
...
def test_backend_defaults_to_default_config_and_torch_inductor_jit_delegate():
    first_backend = KernelSelectorBackend()
    second_backend = KernelSelectorBackend()

    assert isinstance(first_backend._config, KernelSelectorBackendConfig)
...
def test_config_rejects_invalid_values(field, value, match):
    ...
    with pytest.raises((TypeError, ValueError), match=match):
        KernelSelectorBackendConfig(**kwargs)
...
def test_config_deserialization_is_not_supported():
    with pytest.raises(NotImplementedError, match="one-way serialization only"):
        KernelSelectorBackendConfig.from_dict({})
...
def test_create_optimizer_uses_backend_config(mocker):
    ...
    backend = KernelSelectorBackend(config, _RecordingJitDelegate())
    optimizer_class = mocker.patch("aitune.torch.backend.kernel_selector_backend.KernelOptimizer")
...
def test_synchronize_plan_restores_rank_zero_plan_on_nonzero_rank(mocker):
    ...
    broadcast = mocker.patch(
        "aitune.torch.backend.kernel_selector_backend.coordinator.broadcast_from_rank0",
...
def test_checkpoint_restores_inactive_jit_runtime_without_optimizer(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aitune.torch.backend.kernel_selector_backend.KernelOptimizer", _UnexpectedOptimizer)
    restored_module = _ReluModule()
    restored = KernelSelectorBackend.from_dict(restored_module, state)
...
def test_checkpoint_jit_runtime_follows_activate_and_deactivate(tmp_path):
    ...
    backend = _backend(_RecordingJitDelegate(), _PlanOptimizer(plan))
    _, sample, _ = _build(backend, tmp_path)
    restored = KernelSelectorBackend.from_dict(_ReluModule(), backend.to_dict())
...
def test_checkpoint_restores_aot_delegate_without_module(tmp_path):
    ...
    restored = KernelSelectorBackend.from_dict(None, backend.to_dict())
    ...
    assert KernelSelectorBackend.STATE_KERNEL_PLAN not in backend.to_dict()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"type": "KernelSelectorBackend", "version": 2}, "Unsupported KernelSelectorBackend state version"),
        (
            {"type": "KernelSelectorBackend", "version": 1, "kernel_plan": {}},
...
def test_checkpoint_rejects_invalid_parent_state(state, match):
    with pytest.raises(ValueError, match=match):
        KernelSelectorBackend.from_dict(None, state)


def test_checkpoint_rejects_unknown_delegate_type():
    state = {
        "type": "KernelSelectorBackend",
        "version": 1,
...
def test_checkpoint_requires_module_for_jit_delegate(tmp_path):
    ...
    with pytest.raises(ValueError, match="Module is required to restore a just-in-time delegate"):
        KernelSelectorBackend.from_dict(None, backend.to_dict())

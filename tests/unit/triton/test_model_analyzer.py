...
def test_analyzer_uses_only_enabled_tensorrt_profiles(tmp_path):
    ...
    artifact = replace(
        ...
        optimization_profiles=artifact.optimization_profiles
        + (
            TensorRTOptimizationProfile(
                inputs=(TensorRTProfileInput(name="input", min_shape=(1, 16), opt_shape=(4, 16), max_shape=(8, 16)),)
            ),
        ),
    )
    model = aitriton.publish(artifact, path=tmp_path / "repository", model_name="encoder", max_batch_size=8)
...
def test_analyzer_requires_a_common_batch_range_for_all_inputs(tmp_path, compatible_fallback):
    artifact = _plan(tmp_path / "source.plan")
    incompatible = TensorRTOptimizationProfile(
        inputs=(
            TensorRTProfileInput(name="input", min_shape=(1, 8), opt_shape=(2, 8), max_shape=(2, 8)),
            TensorRTProfileInput(name="mask", min_shape=(3, 8), opt_shape=(4, 8), max_shape=(8, 8)),
        )
    )
    compatible = TensorRTOptimizationProfile(
        inputs=tuple(
            TensorRTProfileInput(name=name, min_shape=(4, 8), opt_shape=(4, 8), max_shape=(8, 8))
    ...
    artifact = replace(
        ...
        optimization_profiles=(incompatible, compatible) if compatible_fallback else (incompatible,),
    )
    repository = tmp_path / "repository"

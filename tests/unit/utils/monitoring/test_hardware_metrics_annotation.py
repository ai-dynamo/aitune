...
def test_context_manager(mock_collector):
    with collect_hardware_metrics(name="test_scope", collector=mock_collector):
        pass
...
def test_context_manager_calls_end_scope_on_exception(mock_collector):
    with pytest.raises(ValueError):
        with collect_hardware_metrics(name="test", collector=mock_collector):
...
def test_decorator(mock_collector):
    @collect_hardware_metrics(name="decorated_func", collector=mock_collector)
    def my_func():
        return 42
...
def test_context_manager_without_name_raises(mock_collector):
    with pytest.raises(ValueError, match="name must be provided"):
        with collect_hardware_metrics(collector=mock_collector):
            pass
...
def test_decorator_calls_end_scope_on_exception(mock_collector):
    @collect_hardware_metrics(collector=mock_collector)
    def my_func():
        raise ValueError("test error")
